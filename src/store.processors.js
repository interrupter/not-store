...
class notStoreProcessors{
    ...
    static async run(
        ...
        try{
            ...
            if(e instanceof notError){
                ...
                notNode.Application.report(new notStoreExceptionProccesorRunError(list, filename, driver.name, e));
            }
        }
    }
    ...
    static getProcessorAndOptions(item){
        ...
        if(typeof processorName === 'undefined' || typeof processorOptions === 'undefined'){
    ...
    static getProcessorOptions(item){
        if(Array.isArray(item) && item.length > 1 && typeof item[1] === 'object' && item[1]){
            return item[1];
        }
    ...
    static getProcessorName(item){
        if(typeof item === 'string'){
            return item;
        }else if(Array.isArray(item) && item.length > 0 && typeof item[0] === 'string' && item[0].trim()){
            return item[0].trim();
        }
