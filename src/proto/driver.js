...
const Log = require('not-log')(module, 'driver');

const notStoreProcessors = require('../store.processors');

const {
	OPT_MAX_INPUT_PATH_LENGTH, FIRST_DIR_NAME_LENGTH,
	PROCESSOR_TYPES,
	DEFAULT_FILENAME_SPLIT
} = require('../const');

const { notStoreExceptionPreproccesorOptionsWrongFormat,
	notStoreExceptionLocalFileDeleteError,notStoreExceptionPreproccesorRunError } = require('../exceptions');
...
class notStoreDriver{
    ...
    getPrefixDirName(fname){
        return fname.substring(0, FIRST_DIR_NAME_LENGTH);
    }
	...
	getFullPathToFileFromFilename(fname) {
        return (
            ...
            "/" +
            fname.substring(FIRST_DIR_NAME_LENGTH)
        );
    }

	getPathInStore(filename) {
		let pathInStore = '';
		//groouping files in folders named as first N chars of filename 123/4567.ext
		if (this.options.groupFiles) {			
			let end = filename?this.getPrefixDirName(filename):false;
			if (this.options.path) {
				pathInStore = end?path.join(this.options.path, end):this.options.path;
			} else {
				pathInStore = end;
			}
		} else {
			if (this.options.path) {
				pathInStore = this.options.path;
			}
		}
		return pathInStore;
	}
	...
	getFilename(uuid, postfix, format) {
		return uuid + (postfix ? (DEFAULT_FILENAME_SPLIT + postfix) : '') + (format ? ('.' + format) : '');
	}
	...
	getFullFilename(uuid, postfix, format) {
		return path.join(this.getPathInStore(uuid), this.getFilename(uuid, postfix, format));
	}
	...
	getVariantPath(srcParts, variantShortName, variant){
		return path.join(srcParts.dir, this.getVariantFilename(srcParts, variantShortName, variant));
	}
    ...
    getVariantsPaths(src, variants, format) {
		...
		Object.keys(variants).forEach((variantShortName) => {
			result[variantShortName] = {
				...
				local: 		this.getVariantPath(srcParts, variantShortName, variants[variantShortName]),
				filename:	this.getVariantFilename(srcParts, variantShortName, variants[variantShortName])
			};
		});
		return result;
	}
    ...
    async readableStreamFromString(source){
        ...
        if (source.length < OPT_MAX_INPUT_PATH_LENGTH) {
            ...
            if (isUrl.isUri(encodeURI(source))) {
                ...
                } else { //short line but not URL and not file path === stringified file
	...
	async removeLocalFile(filename) {
		try{
			...
			notNode.Application.report(new notStoreExceptionLocalFileDeleteError(filename, e ));
		}		
	}
	...
	isProcessorsSet(processorType , action){
		return Object.hasOwn(this.#options, 'processors') 
			&& Object.hasOwn(this.#options.processors, action)
			&& Object.hasOwn(this.#options.processors[action], processorType) 
			&& Array.isArray(this.#options.processors[action][processorType]);
	}
	...
	getProcessors(processorType, action){
		return this.#options.processors[action][processorType];
	}

	async runPreProcessors(action, filename, metadata){
		return this.runProcessors(PROCESSOR_TYPES.PROCESSOR_TYPE_PRE, action, filename, metadata);
	}


	async runPostProcessors(action, filename, metadata){
		return this.runProcessors(PROCESSOR_TYPES.PROCESSOR_TYPE_POST, action, filename, metadata);
	}
