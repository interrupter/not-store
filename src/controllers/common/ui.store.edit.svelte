<script>
    ...
    function collectData() {
        ...
        MODES_FIELDS[mode].forEach((fieldname) => {
            if (Object.hasOwn(fields, fieldname) && fields[fieldname].enabled) {
        ...
        if (MODES.indexOf(mode) > -1) {
            ...
            fieldsList.forEach((fieldName) => {
                if (fields[fieldName].enabled && fields[fieldName].required) {
                    let val =
                        freshData && freshData.field === fieldName
                            ? freshData.value
    ...
    $: nameHelper = validationErrors.name
        ? validationErrors.name.join(", ")
        : fields.name.placeholder;
    $: nameClasses = validationErrors.name ? CLASS_ERR : CLASS_OK;

    $: optionsHelper = validationErrors.options
        ? validationErrors.options.join(", ")
        : fields.options.placeholder;
    $: optionsClasses = validationErrors.options ? CLASS_ERR : CLASS_OK;

    $: driverHelper = validationErrors.driver
        ? validationErrors.driver.join(", ")
        : fields.driver.placeholder;
    ...
    <div class="container">
            ...
            <div class="field edit-form-field edit-form-name">
                ...
                <div class="control has-icons-right">
                    <input
                        ...
                        on:input={onInput}
                        autocomplete="name"
                        aria-controls="input-field-helper-name"
                        aria-describedby="input-field-helper-name"
                    />
        ...
        {#if fields.driver.enabled}
            <div class="field edit-form-field edit-form-driver">
                ...
                <div class="control">
                    <div class="select {driverClasses}">
                        <select
                            bind:value={fields.driver.value}
                            name="driver"
                            on:blur={onChange}
                            on:input={onInput}
                        >
            ...
            {#if fields.driver.value}
                <svelte:component
                    this={COMPONENTS.get(
                        Common.DRIVERS[fields.driver.value].ui
                    )}
                    bind:value={options.driver}
                />
				{#each Common.DRIVERS[fields.driver.value].actions }
            {/if}
        {/if}
        ...
        {#if fields.options.enabled}
            <div class="edit-form-field edit-form-options field">
                ...
                <div class="control has-icons-right">
                    <textarea
                        invalid={validationErrors.options}
                        on:change={onChange}
                        on:input={onInput}
                        ...
                        rows="10"
                        aria-controls="input-field-helper-options"
                        aria-describedby="input-field-helper-options"
                    />
                ...
                <p
                    class="help {optionsClasses}"
                    id="input-field-helper-options"
                >
        ...
        {#if fields.active.enabled}
            <div class="edit-form-field edit-form-active field">
                <input
                    ...
                    aria-describedby="input-field-helper-active"
                />
                <label class="label" for="user-login-form-active-field"
                    >{fields.active.label}</label
                >
        ...
        <div class="buttons-row">
            {#if cancel.enabled}
                <button
                    class="button is-outlined edit-form-cancel"
            ...
            {#if submit.enabled}
                <button
                    ...
                    class="button is-primary is-hovered edit-form-submit pull-right"
