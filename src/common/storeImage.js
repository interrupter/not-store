...
class notStoreImage {
	...
	*/
    ...
    getFullName(name, thumb, ext) {
        ...
        return (
            path.join(this.options.root, pathName) +
            (thumb ? "." + thumb : "") +
            "." +
            (ext ? ext : OPT_DEFAULT_EXTENSION)
        );
    }
    ...
    getThumbsNames(name) {
        ...
        if (this.options.thumbs) {
            for (let thumb in this.options.thumbs) {
                names[thumb] = this.getFullNameFromRoot(
                    name,
                    thumb,
                    this.options.extension || OPT_DEFAULT_THUMB_EXTENSION
                );
            }
        }
        return names;
    ...
    getPaths(meta) {
        ...
        if (this.options.thumbs) {
            for (let thumb in this.options.thumbs) {
                paths.thumb[thumb] = this.getFullNameFromRoot(
                    meta.name,
                    thumb,
                    this.options.extension || OPT_DEFAULT_THUMB_EXTENSION
                );
        ...
        if (fullName) {
            try {
                let image = sharp(fullName),
                    thumbFullName = this.getFullName(
                        name,
                        thumb,
                        this.options.extension || OPT_DEFAULT_THUMB_EXTENSION
                    );
                ...
                image
                    .resize(
                        profile.width || profile.max,
                        profile.height || profile.max,
                        { fit: "outside" }
                    )
                    .toFile(thumbFullName);
            } catch (e) {
                log.error(e);
            }
        }
            ...
            this.stashFile(file)
                .then((metadata) => {
                    let name = store.createFileName(),
                        ...
                        fullNameFromRoot = this.getFullNameFromRoot(
                            name,
                            null,
                            metadata.format
                        ),
	...
	*/
    get() {
        ...
        if (arguments.length == 2) {
            ...
            fullName = this.resolveFileFullName(
                arguments[0],
                arguments[1] === OPT_ORIGINAL_THUMB ? undefined : arguments[1]
            );
