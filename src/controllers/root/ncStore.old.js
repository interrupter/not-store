import DEFAULT_OPTIONS from "../../drivers/timeweb.driver.options";

import { Frame, Elements } from "not-bulma";

const { notBreadcrumbs, notTable, notController, notCommon } = Frame;

const { UIError } = Elements.Notifications;

import Common from "not-store/src/controllers/common/lib";
...
class ncStore extends notController {
    ...
    route(params = []) {
        if (params.length == 1) {
            ...
            if (params[1] === "delete") {
                ...
                let routeRunnerName =
                    "run" + notCommon.capitalizeFirstLetter(params[1]);
                if (
                    this[routeRunnerName] &&
                    typeof this[routeRunnerName] === "function"
                ) {
    ...
    goCreate() {
        this.app
            .getWorking("router")
            .navigate([this.getModelURL(), "create"].join("/"));
    }

    goDetails(value) {
        this.app
            .getWorking("router")
            .navigate([this.getModelURL(), value].join("/"));
    }

    goUpdate(value) {
        this.app
            .getWorking("router")
            .navigate([this.getModelURL(), value, "update"].join("/"));
    }

    goDelete(value) {
        this.app
            .getWorking("router")
            .navigate([this.getModelURL(), value, "delete"].join("/"));
    }
